...
class InstallCreator:
    def __init__(self, name : str, version : str, title : str, publisher : str, startmenudir : str = None, targetdir : str = None):
        self.__OutputName = 'Installer.exe'
        self.Name = name
    ...
    def addPackage(self, page : InstallerPackageInfoBase):
    ...
    def __getInstallerBinaryPath(self):
        qtinstallerframework = ConfigObj.inst().getQtInstallerFramework()
        binpath = os.path.join(qtinstallerframework, 'bin')
        return binpath
    ...
    def __binarycreator(self, sourcepath):
        ...
        args = '{0} -c config\config.xml --online-only -p packages {1}'.format(binarycreatorpath, self.__OutputName)
